...
const schema = z.object({
  ...
  population: z.object({ powerShares: z.object({ low: z.number().min(0).max(1), mid: z.number().min(0).max(1), high: z.number().min(0).max(1) }), basePower: z.object({ low: positive, mid: positive, high: positive }), powerSigma: z.object({ low: nonNegative, mid: nonNegative, high: nonNegative }), weakFormationScale: z.object({ low: positive.max(1), mid: positive.max(1), high: positive.max(1) }) }).refine(({ powerShares }) => Math.abs(powerShares.low + powerShares.mid + powerShares.high - 1) < 1e-6, "power shares must total 1"),
  ...
  strategy: z.object({ shares: z.object({ centerRush: z.number().min(0).max(1), supportExpand: z.number().min(0).max(1), multiFront: z.number().min(0).max(1) }).refine(({ centerRush, supportExpand, multiFront }) => Math.abs(centerRush + supportExpand + multiFront - 1) < 0.0001, "strategy shares must total 1"), activityWeight: nonNegative, powerWeight: nonNegative, randomWeight: nonNegative, centerWeight: nonNegative, resourceWeight: nonNegative, normalWeight: nonNegative, congestionAvoidance: z.number().min(0).max(2) }),
